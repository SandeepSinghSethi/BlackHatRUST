...
use reqwest::Client; //using async functionality , so no blocking client used
...
pub async fn enumerate(http_client :&Client, target: &str) -> Result<Vec<Subdomain> ,Error> {
    ...
    let dns_resovler = TokioAsyncResolver::tokio(
    ...
    let subdomains: Vec<Subdomain> = stream::iter(subdomains.into_iter())
        ...
        .filter_map(|subdomain|{
            let dns_resolver = dns_resovler.clone();
            async move{
...
pub async fn resolves(dns_resolver : &TokioAsyncResolver,domain: &Subdomain) -> bool {
